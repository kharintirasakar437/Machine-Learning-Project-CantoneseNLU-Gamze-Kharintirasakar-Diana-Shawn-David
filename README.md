...
| Complex | Gradient boosting or MLP | Optional, only if feasibility is confirmed |
...
- **The test split is used once**, for the final reported evaluation.
- **Fixed seed 3052** everywhere; at least three seeds where results are stochastic, with
  variability reported.
- TF-IDF vectorizers, scalers, and other fitted preprocessing tools are fitted on **training data
  only**, inside an sklearn `Pipeline`.
- The primary metric is **macro-F1**, chosen for cross-task consistency and per-class visibility,
  *not* for class imbalance (the sentiment set is deliberately balanced, so accuracy is reported
  alongside).
...
The baseline training interface will be added during Milestone 3 (internal deadline Oct 13).
...
Final evaluation will use the locked test split only after model selection is complete. Run this
**once**, for the report:
...
`--final` will print a warning by design: it touches the locked test split. The exact command
structure may change as the implementation is developed.
...
Every deliverable is finished at least 48 hours before its official milestone date. The team meets
Mondays 20:00–21:00; day-to-day communication is on Discord.
...
- AI assistance is recorded in [`AI_USE.md`](AI_USE.md) at the time of use. Generative models studied
  as an *object of the experiment* are logged separately from AI assistance.
...
Human-performance measurement is outside the scope of this CSCI 3052U course project; it belongs to
